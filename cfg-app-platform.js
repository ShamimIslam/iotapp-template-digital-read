 ...
 *     altGpio:  33            // initialize alternate GPIO port as input
 * } ;
    ...
    cfg.init = function() {
        ...
        switch( cfg.mraa.getPlatformType() ) {          // which board are we running on?
            ...
            case cfg.mraa.INTEL_GT_TUCHUCK:             // Joule (aka Grosse Tete)
                gpio = opt.altGpio ? gpio : 35 ;        // use alternate I/O pin?
                cfg.gpio = new cfg.mraa.Gpio(gpio) ;    // initialize digital I/O pin
                break ;

            default:
                if( opt.skipTest && opt.altGpio ) {
                    cfg.gpio = new cfg.mraa.Gpio(gpio) ; // force run on unknown platform with alt pin
                }
                else {
